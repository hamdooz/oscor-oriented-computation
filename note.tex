...
computations below were carried out with OSCAR (Julia~1.12,
\texttt{Oscar.jl} as installed from the General registry in July~2026)
and can be reproduced verbatim by pasting the listed code into a Julia
session after \texttt{using Oscar}. The complete, individually runnable
source code for every computation in this note, together with a
\texttt{Project.toml} pinning the OSCAR dependency and instructions for
generating a \texttt{Manifest.toml} with the exact resolved package
versions on the reader's own machine, is available in the accompanying
repository.\footnote{\url{https://github.com/REPLACE-WITH-USERNAME/annihilator-dependency-oscar-verification}
(placeholder -- to be created and filled in before submission).}
